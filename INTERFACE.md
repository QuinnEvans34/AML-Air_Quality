...
**Your decision:** We are going to be training the model every week, and triggering a re-train based on our F1 metric. Starting in W7A1 we add a third trigger: per-location PM2.5 drift greater than 2.0 standard deviations from the training reference distribution also forces a retrain.
...
**Drift threshold (W7A1 addition).** The drift check task compares a recent window of raw PM2.5 (today's `pm25_{ds}.csv`) against the prior `DRIFT_REFERENCE_DAYS` (= 7) days of raw PM2.5 per location, computes `mean_shift_sigma = (recent.mean() - reference.mean()) / reference.std()`, and flags a location as drifted when `abs(mean_shift_sigma) > DRIFT_SIGMA_THRESHOLD` (= 2.0). When a location is drifted, the DAG retrains that location regardless of its prior F1. The 2σ threshold is the leading-indicator partner to the 0.70 F1 lagging-indicator floor — 2σ catches an upstream covariate shift (wildfire smoke arrival, a multi-day inversion) before downstream F1 has time to fully collapse, while F1 < 0.70 still catches outcome degradation in cases where the input distribution stays stable but the label boundary becomes mis-calibrated. The two triggers overlap deliberately: if 2σ drift sustains for several days the corresponding F1 will fall below 0.70 within roughly one retrain cycle, so the input-side and outcome-side triggers point at the same retrain decision rather than contradicting it. A 3σ cut would only fire on catastrophic shifts and would let real drift go unflagged until F1 had already collapsed; a 1σ cut would fire on routine noise and force unnecessary retrains. The full design (per-location verdict JSON shape, MLflow run shape, sparse-window guards, idempotency) lives in `docs/drift_implementation_plan.md`.
...
| 2026-05-14 | Added `check_drift` task between `engineer_features` and `retrain_model` (5-task chain). Per-location `mean_shift_sigma` and `drifted` flag are logged to MLflow under run name `drift_{ds}` and persisted to `include/data/drift/drift_{ds}.json`. Drift > `DRIFT_SIGMA_THRESHOLD` (2.0) added as a third retrain trigger in `_per_location_decisions` (precedence: Monday → bootstrap → drift → F1 < 0.70 → skip). New constants `DRIFT_SIGMA_THRESHOLD`, `DRIFT_REFERENCE_DAYS`, `DRIFT_RECENT_DAYS` added to Shared Constants. Source-of-truth design lives in `docs/drift_implementation_plan.md`. | W7A1 Part 2 requires drift detection between engineer and retrain, with mean shift logged to MLflow and a documented threshold consistent with the retraining trigger. | Yes |
| 2026-05-14 | `train.py.log_run_to_mlflow` now promotes each newly registered model version to MLflow's Production stage and archives the prior Production version (best-effort, warning on failure). Module-level `logger` introduced so `log_run_to_mlflow` and `retrain_task` share one logger. No other module touched. Source-of-truth design lives in `docs/serve_production_promotion_plan.md`. | Cold-starting `uvicorn serve:app` previously failed because no version was ever at the Production stage; W7A1 Part 5 end-to-end verification step 3 requires Production-stage models. | Yes |
...
| 2026-05-14 | Implemented the AirAlert dashboard at `app/dashboard/` as a Next.js + React + Tailwind project. Adds four API routes (`/api/health`, `/api/predict`, `/api/features`, `/api/trend`) — the first two proxy `serve.py`, the last two run server-side feature/trend prep against `include/data/raw/`. The browser never talks to FastAPI directly. UI components implement the three-color verdict strip (safe/borderline/unsafe), Recharts trend chart with 35.4 reference line, plain-language headline grouping consecutive unsafe hours, and a polling HealthBadge. Visual spec in `docs/dashboard_ui_spec.md`; architecture spec in `docs/dashboard_implementation_plan.md`. | W7A1 Part 4 — user-facing dashboard. Pairs with the train.py Production promotion fix in Phase 1a so the lifespan loader inside `serve.py` resolves `models:/.../Production` on cold start. | Yes |
| 2026-05-14 | `train.py.train_logistic_regression` gains a single-class fallback: when `y_train.nunique() < 2` (the chronological 80/20 split occasionally puts every unsafe hour into the test set given the ~0.6% positive class rate), it returns a pre-fit `DummyClassifier(strategy="constant", constant=<observed>)` whose `classes_` array contains both 0 and 1 so `serve.py` accepts it unchanged. Emits a WARNING log line so the data shortage is visible. Resulting model's F1 on the unsafe class is 0.0 by construction — degenerate but transparent. | The previous code raised `sklearn.exceptions.ValueError("This solver needs samples of at least 2 classes")` and crashed the DAG, blocking the Phase 1a Production promotion from running. The fallback keeps the DAG green and surfaces the issue without papering over it. | Yes |
